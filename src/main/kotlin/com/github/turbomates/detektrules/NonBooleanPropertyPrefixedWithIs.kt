 ...
 *
 * The difference from original rule is that you can specify excluded types which indirectly represent boolean (e.g. column wrapper)
 *
 * <noncompliant>
...
class NonBooleanPropertyPrefixedWithIs(config: Config = Config.empty) : Rule(config) {
    ...
    @Configuration("Types which for some reason should be treated as boolean.")
    private val excludedTypes: List<String> by config(emptyList())
    ...
    private fun validateDeclaration(declaration: KtCallableDeclaration) {
        ...
        if (name.startsWith("is") && name.length > 2 && !name[2].isLowerCase()) {
            ...
            val isNotBooleanType = typeName != kotlinBooleanTypeName && typeName != javaBooleanTypeName && !excludedTypes.contains(typeName)
